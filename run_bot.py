...
from setup_bots import create_bot_config, load_bot_configs
...
def sync_bot_config(bot_num: int) -> Path | None:
    ...
    if not config.get("TOKEN"):
        return None
    return create_bot_config(bot_num, config)
...
def main():
    ...
    bot_num_raw = sys.argv[1]
    if not bot_num_raw.isdigit():
        print(f"❌ 无效的bot编号: {bot_num_raw}")
        return 1

    bot_num = int(bot_num_raw)
    synced_config_dir = sync_bot_config(bot_num)
    config_dir = synced_config_dir or Path(f"configs/bot_{bot_num}")
